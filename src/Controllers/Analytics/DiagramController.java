...
public class DiagramController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        for (Produce produce :
                produceList) {
            choiceboxProduction.getItems().add(produce.getName());
        }
    }
    ...
    public void Produce_Chosen(ActionEvent actionEvent) {
        ...
        series2.getData().add(new XYChart.Data<>("Внешний вид", produce.getfStandart().getAppearance()));
        series2.getData().add(new XYChart.Data<>("Размер", produce.getfStandart().getSize()));
        series2.getData().add(new XYChart.Data<>("Вес", produce.getfStandart().getWeight()));
        series2.getData().add(new XYChart.Data<>("Толщина", produce.getfStandart().getThickness()));
        series2.getData().add(new XYChart.Data<>("Прочность", produce.getfStandart().getStrength()));
