...
public class ProduceController  implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (produces.size() != 0 && produces!=null)
            for (Produce produce :
                produces) {
            TableProduce tableProduce =new TableProduce(produce.getId(),produce.getName(),produce.getStrength(),produce.getWeight(),produce.getSize(),produce.getThickness(),produce.getAppearance(),produce.getfStandart().getName(),produce.getfBatch().getId(),produce.getfWorker().getFIO());
