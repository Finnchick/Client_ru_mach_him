...
public class BatchController implements Initializable {
    ...
    public void Delete_Pressed(ActionEvent actionEvent) throws IOException {
        ...
        TableBatch tableBatch = tableview.getSelectionModel().getSelectedItem();
        requestModel.setRequestMessage(new Gson().toJson(new Produce(tableBatch.getId())));
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (batches.size() != 0 && batches != null)
            for (Batch batch :
                    batches) {
                TableBatch tableBatch = new TableBatch(batch.getId(), String.valueOf(batch.getDate()), batch.getfWorker().getFIO(), batch.getfUser().getLogin());
                list.add(tableBatch);
            }
