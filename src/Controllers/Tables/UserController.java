...
public class UserController implements Initializable {
    ...
    public void Delete_Pressed(ActionEvent actionEvent) throws IOException {
        ...
        requestModel.setRequestMessage(new Gson().toJson(new Standart(tableUser.getId())));
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (users.size() != 0 && users != null)
            for (User user :
                    users) {
                TableUser tableUser = new TableUser(user.getId(), user.getLogin(), user.getPassword(), String.valueOf(user.getRole()));
                list.add(tableUser);
            }
