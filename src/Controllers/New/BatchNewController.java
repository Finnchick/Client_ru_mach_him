...
public class BatchNewController implements Initializable {
    ...
    public void Ok_Pressed(ActionEvent actionEvent) throws IOException {
        try {
            ...
            Request request;
            batch.setfWorker(new Worker((Integer) choiceboxWorker.getSelectionModel().getSelectedItem()));
            batch.setfUser(new User((Integer) choiceboxUser.getSelectionModel().getSelectedItem()));
    ...
    public void initialize(URL location, ResourceBundle resources) {
        try {
            ...
            for (User user :
                    users) {
                choiceboxUser.getItems().add(user.getId());
            }
            if (ClientSocket.getInstance().getBatchId() != -1) {
                ...
                choiceboxUser.setValue(batch.getfUser().getId());
                choiceboxWorker.setValue(batch.getfWorker().getId());
            }
        } catch (Exception e) {
