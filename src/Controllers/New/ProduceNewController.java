...
public class ProduceNewController implements Initializable {
    ...
    public void Ok_Pressed(ActionEvent actionEvent) throws IOException {
        try {
            ...
            Request request;
            produce.setfWorker(new Worker((Integer) choiceboxWorker.getSelectionModel().getSelectedItem()));
            produce.setfBatch(new Batch((Integer) choiceboxBatch.getSelectionModel().getSelectedItem()));
            produce.setfStandart(new Standart((Integer) choiceboxStandart.getSelectionModel().getSelectedItem()));
            produce.setAppearance((Integer) spinnerAppearance.getValue());
            produce.setName(textfieldName.getText());
    ...
    public void initialize(URL location, ResourceBundle resources) {
        try {
            ...
            for (Worker worker :
                    workers) {
                choiceboxWorker.getItems().add(worker.getId());
            }
            for (Batch batch :
                    batches) {
                choiceboxBatch.getItems().add(batch.getId());
            }
            for (Standart standart :
                    standarts) {
                choiceboxStandart.getItems().add(standart.getId());
            }
            if (ClientSocket.getInstance().getProduceId() != -1) {
                ...
                textfieldName.setText(produce.getName());
                ...
                spinnerWeight.setValueFactory(valueWeightFactory);
                textfieldName.setText(produce.getName());
                choiceboxWorker.setValue(produce.getfWorker().getId());
                choiceboxStandart.setValue(produce.getfStandart().getId());
                choiceboxBatch.setValue(produce.getfBatch().getId());
            }
        } catch (Exception e) {
